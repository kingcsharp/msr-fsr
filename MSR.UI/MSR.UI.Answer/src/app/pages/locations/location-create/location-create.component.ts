...
export class LocationCreateComponent implements OnInit {
        ...
        responseHandler((response) => {
          if (this.locationToEdit !== undefined) {
            this.parentLocationOptions = response.object
              .filter((s) => s.id !== this.locationToEdit.id)
          ...
          this.timezoneService.timezone(env.apiVersion).subscribe(
            responseHandler((timezoneResponse) => {
              this.timezonesAvailable = timezoneResponse.object.map((s) => ({
                label: s.description,
                value: s.id,
              }));
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      if (this.locationToEditId !== 0) {
        ...
        this.locationService
          ...
          .subscribe(
            responseHandler((locationGetResponse) => {
              ...
              this.globals.showLoader(true);
              this.sensorService
                .sensor(null, this.locationToEdit.site, env.apiVersion)
                .subscribe(
                  responseHandler((locationResponse) => {
  ...
  updateLocation() {
    ...
    updateLocationRequest.timeZoneId =
      this.selectedTimezone === undefined || this.selectedTimezone === null
        ? null
        : this.selectedTimezone;
    this.globals.showLoader(true);
    this.locationService
      .locationPatch(env.apiVersion, updateLocationRequest)
      .subscribe(
        responseHandler((response) => {
          ...
          if (this.selectedSensors.length !== 0) {
            this.originalSensorsSelected.forEach((originalSensor) => {
              if (
                this.selectedSensors.find((s) => s.id === originalSensor.id) ===
                undefined
              ) {
            ...
            this.selectedSensors.forEach((selectedSensor) => {
              if (
                this.originalSensorsSelected.find(
                  (s) => s.id === selectedSensor.id
                ) === undefined
              ) {
            ...
            sensorsToDelete.forEach((sensorToDelete) => {
              ...
              this.locationService
                ...
                .subscribe(responseHandler((sensorResponse) => {}));
            });
  ...
  saveLocation() {
    ...
    createLocationRequest.timeZoneId =
      this.selectedTimezone === undefined || this.selectedTimezone === null
        ? null
        : this.selectedTimezone;
    this.globals.showLoader(true);
    this.locationService
      .locationPost(env.apiVersion, createLocationRequest)
      .subscribe(
        responseHandler((response) => {
          ...
          if (this.selectedSensors.length !== 0) {
            this.selectedSensors.forEach((sensor) => {
              ...
              this.locationService
                .sensorPost(
                  Number(this.locationToEditId),
                  Number(sensor.id),
                  env.apiVersion
                )
                .subscribe(responseHandler((sensorPostResponse) => {}));
            });
          }
  ...
  parentSelected($event) {
    if ($event.value === null) {
      ...
      this.sensorService
        .sensor(null, $event.value.site, env.apiVersion)
        .subscribe(
          responseHandler((response) => {
