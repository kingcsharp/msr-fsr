...
export class OperationsComponent implements OnInit {
  ...
  getReportData() {
      ...
      .subscribe(
        responseHandler((response) => {
          this.reportInfo = response.object.filter(
            (x) => x.id === EnumReport.RevenuebyCustomerbyTimePeriod
          )[0];
      ...
      .subscribe(
        responseHandler((response) => {
          this.reportInfo2 = response.object.filter(
            (x) => x.id === EnumReport.RevenuebyKitbyPartKit
          )[0];
    ...
    this.reportService
      ...
      .subscribe(
        responseHandler((response) => {
          this.reportInfo3 = response.object.filter(
            (x) => x.id === EnumReport.CountofKitsbyPartKit
          )[0];
