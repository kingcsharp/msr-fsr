...
import {
  ...
  WorkflowGroupModel,
  WorkflowGroupRoleMapModel,
  RoleService,
  UserService,
  WorkflowGroupUserMapModel,
  Role,
  AuditActionResultOfWorkflowGroupModel,
...
export class ApprovalGroupsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.gridStorageId =
      "workflowGroupGrid" + this.elem.nativeElement.tagName.toLowerCase();
    this.gridSettings = [
  ...
  getWorkflowGroups(event: LazyLoadEvent) {
    ...
    setTimeout(() => {
      callFunctionWithFilters(
        ...
        this.workflowGroupService.workflowGroupGet,
        event,
  ...
  workflowGroupStatus(workflowGroup: WorkflowGroupModel) {
    ...
    const updateWorkflow = new UpdateWorkflowGroupRequest({
      ...
      roles: workflowGroup.groupRoles,
      isActive: workflowGroup.isActive,
    });
    this.globals.showLoader(true);
    this.workflowGroupService
      .workflowGroupPatch(env.apiVersion, updateWorkflow)
      .pipe(take(1))
      .subscribe(
        responseHandler(
          (resp) => {
  ...
  removeRow(workflowGroup) {
    ...
    this.globals.showLoader(true);
    this.workflowGroupService
      .workflowGroupDelete(workflowGroup.id, env.apiVersion)
      .pipe(take(1))
      .subscribe(
        responseHandler(
          (resp) => {
  ...
  onWorkflowSubmit() {
    ...
    if (jQuery(".parsleyjs").parsley().isValid()) {
      ...
      this.currWorkflowGroup.groupRoles.forEach((x) => {
        groupRoles.push(
          new WorkflowGroupRoleMapModel({
            roleId: x.roleId,
            workflowGroupId: this.currWorkflowGroup.id,
          })
        );
      });
      ...
      if (this.currWorkflowGroup.id === undefined) {
        const createWorkflow = new CreateWorkflowGroupRequest({
          name: this.currWorkflowGroup.name,
          users: groupUsers,
          roles: groupRoles,
          isActive: this.currWorkflowGroup.isActive,
        });
        method = this.workflowGroupService.workflowGroupPost(
          env.apiVersion,
          createWorkflow
        );
      } else {
        ...
        method = this.workflowGroupService.workflowGroupPatch(
          env.apiVersion,
          updateWorkflow
        );
      }
            ...
            if (!resp.hasErrors) {
              if (ctrl.currWorkflowGroup.id === undefined) {
                ...
                const index = ctrl.data.findIndex(
                  (x) => x.id === ctrl.currWorkflowGroup.id
                );
