...
export class Login {
  ...
  public async forgotUserPassword() {
    ...
    this.accountService
      .forgotpassword(
        env.apiVersion,
        new ForgotPasswordRequest({ userName: this.username })
      )
      .pipe(take(1))
      .subscribe(
        responseHandler(
  ...
  public async forgotUserName() {
    ...
    this.accountService
      .forgotusername(
        env.apiVersion,
        new ForgotUserNameRequest({ email: this.email })
      )
      .pipe(take(1))
      .subscribe(
        responseHandler(
          () => {
            this.loginService.isFetching = false;
            ctrl.loginService.loginError(
              "An email has been sent with your information."
            );
            ctrl.showLoginDiv();
          },
          () => {
