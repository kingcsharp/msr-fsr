...
export class PurchaseOrdersComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.purchasePrivileges = this.globals.getEnumPrivileges(
      this.menuItems.Purchases
    );
    this.purchaseOrderPrivileges = this.globals.getEnumPrivileges(
      this.menuItems.PurchaseOrders
    );
    this.data = [];
    this.getCustomers();
  }
  ...
  getPurchaseOrders(event: LazyLoadEvent) {
    ...
    setTimeout(() => {
      callFunctionWithFilters(
        ...
        this.purchaseOrderService.view,
        event,
  ...
  showPurchaseOrderModal(purchaseOrder: PurchaseOrderView) {
    if (purchaseOrder === undefined) {
      ...
      this.globals.showLoader(true);
      this.productService
        .getPurchaseOrderProduct(null, purchaseOrder.customerId, env.apiVersion)
        .pipe(take(1))
        .subscribe(
          responseHandler((response) => {
            this.globals.showLoader(true);
  ...
  setCurrentCustomer(currentPO, purchaseOrder) {
    currentPO.customer = this.customersData.find(
      (x) => x.id === purchaseOrder.customerId
    );
  }
  ...
  deletePurchaseOrder() {
    this.globals.showLoader(true);
    this.purchaseOrderService
      .purchaseOrderDelete(this.poToCloseOrDelete.id, env.apiVersion)
      .pipe(take(1))
      .subscribe(
        responseHandler((response) => {
          const index = this.data.findIndex(
            (x) => x.id === this.poToCloseOrDelete.id
          );
  ...
  closePurchaseOrder() {
    ...
    this.purchaseOrderService
      ...
      .pipe(take(1))
      .subscribe((response) => {
        purchaseOrderRequest.products = response.object[0].products.map(
          (elem) => elem.id
        );
        purchaseOrderRequest.closeDate = new Date();
        purchaseOrderRequest.closePurchaseOrder = true;
        this.purchaseOrderService
          .purchaseOrderPatch(env.apiVersion, purchaseOrderRequest)
          .pipe(take(1))
          .subscribe(
            responseHandler((purchaseOrderResponse) => {
              const index = this.data.findIndex(
                (x) => x.id === this.poToCloseOrDelete.id
              );
              this.data.splice(index, 1);
    ...
    this.purchaseOrderService
      ...
      .subscribe((response) => {
        ...
        response.object[0].products.forEach((product) => {
          purchaseOrder.selectedProducts.push(
            this.productsData.find((x) => x.id === product.productId)
          );
        });
  ...
  getProducts() {
    ...
    this.globals.showLoader(true);
    this.productService
      .getPurchaseOrderProduct(null, this.currentPO.customer.id, env.apiVersion)
      .pipe(take(1))
      .subscribe(
        responseHandler((response) => {
  ...
  onChangeTotalPurchaseLimit($event) {
    ...
    if (jQuery("#totalPurchaseLimit").parsley().isValid()) {
      if ($event.target.value) {
        const totalPurchaseLimit = parseFloat(
  ...
  isPoFormValid() {
    if (
      this.currentPO.openDate === undefined ||
      this.currentPO.openDate === null
    ) {
  ...
  onPurchaseSubmit() {
    ...
    if (jQuery(".parsleyjs").parsley().isValid() && this.isPoFormValid()) {
      ...
      if (this.currentPO.id === undefined) {
        ...
        this.purchaseOrderService
          .purchaseOrderPost(env.apiVersion, purchaseOrderRequest)
          .pipe(take(1))
          .subscribe(
            responseHandler((response) => {
        ...
        this.globals.showLoader(true);
        this.purchaseOrderService
          .purchaseOrderPatch(env.apiVersion, purchaseUpdateOrderRequest)
          .pipe(take(1))
          .subscribe(
            responseHandler((response) => {
              const index = this.data.findIndex(
                (x) => x.id === this.currentPO.id
              );
              this.data.splice(index, 1);
