...
import {
  ...
  LocationModel,
  WorkOrderService,
  AuditActionResultOfInvoiceView,
  CreateInvoiceItemRequest,
  UpdateInvoiceItemRequest,
  InvoiceableWorkOrderView,
...
export class InvoiceComponent implements OnInit {
  ...
  allWorkorders: Array<InvoiceableWorkOrderView> =
    new Array<InvoiceableWorkOrderView>();
  workorders: Array<InvoiceableWorkOrderView> =
    new Array<InvoiceableWorkOrderView>();
  calendarEn: any;
  ...
  ngOnInit(): void {
    ...
    this.gridStorageId =
      "invoiceGrid" + this.elem.nativeElement.tagName.toLowerCase();
    this.gridSettings = [
    ...
    this.gridWoSettings = [
      ...
      new ColumnsSaved({
        id: "customerPurchaseNumber",
        label: "Customer Puchase Number",
        visible: true,
      }),
      new ColumnsSaved({
        id: "customerLineNumber",
        label: "Customer Line",
        visible: true,
      }),
      new ColumnsSaved({
      ...
      new ColumnsSaved({
        id: "actualEndDate",
        label: "Work Order Complete Date",
        visible: true,
      }),
  ...
  filterWorkorders() {
    ...
    if (this.currentInvoice.customerId) {
      this.workorders = this.workorders.filter(
        (x) => x.customerId === this.currentInvoice.customerId
      );
    }
    if (this.currentInvoice.locationId) {
      this.workorders = this.workorders.filter(
        (x) => x.locationId === this.currentInvoice.locationId
      );
    }
  ...
  downloadFilteredInvoices() {
    ...
    if (localStorage[this.gridStorageId] !== undefined) {
      ...
      this.invoiceService
        .download(
          this.getFilterVal(filters, "id"),
          this.getFilterVal(filters, "customerName"),
          this.getFilterVal(filters, "description"),
          this.getFilterVal(filters, "invoiceNumber"),
          this.getFilterVal(filters, "dueDate"),
          this.getFilterVal(filters, "createdOn"),
          this.getFilterVal(filters, "createdByName"),
          this.getFilterVal(filters, "lastUpdatedOn"),
          this.getFilterVal(filters, "lastUpdatedByName"),
          this.getFilterVal(filters, "amount"),
          this.getFilterVal(filters, "statusId"),
          env.apiVersion
        )
        .pipe(take(1))
        .subscribe(
  ...
  showDialog(invoice: InvoiceView) {
    ...
    this.invoiceItemOptions = JSON.parse(
      JSON.stringify(this.currentInvoice.invoiceItems)
    );
    this.getWorkOrders();
    this.isSelectAllWorkOrders = false;
    this.display = true;
  ...
  selectWorkOrder(ev, workorder: InvoiceableWorkOrderView) {
    ...
    if (ev.checked) {
      ...
      this.isSelectAllWorkOrders = false;
      const itemIndex = this.currentInvoice.invoiceItems.findIndex(
  ...
  onChangeWorkorders() {
    ...
    this.workorders.forEach((workorder, index) => {
      const isChecked = this.currentInvoice.invoiceItems.some(
  ...
  getInvoices(event: LazyLoadEvent) {
    setTimeout(() => {
      ...
      callFunctionWithFilters(
        this.invoiceService,
        this.invoiceService.invoiceGet,
        event,
  ...
  onInvoiceSubmit() {
    ...
    if (isValid) {
      ...
      if (this.currentInvoice.id !== undefined) {
        ...
        basicReqData.invoiceItems = this.currentInvoice.invoiceItems.map(
          (item) => {
            return new UpdateInvoiceItemRequest({ id: item.workOrderId });
          }
        );
        method = this.invoiceService.invoicePatch(
          env.apiVersion,
          new UpdateInvoiceRequest(basicReqData)
        );
      } else {
        basicReqData.invoiceItems = this.currentInvoice.invoiceItems.map(
          (item) => {
        ...
        if (this.combineSelected) {
          method = this.invoiceService.createOneInvoice(
            env.apiVersion,
            new CreateInvoiceRequest(basicReqData)
          );
        } else {
          this.invoiceService
  ...
  getInvoice(invoice) {
    ...
    invoiceCopy.location = this.locations.find(
      (x) => x.id === invoice.locationId
    );
    invoiceCopy.customer = this.customers.find(
      (x) => x.id === invoice.customerId
    );
    return invoiceCopy;
  }
}
