...
export class TemplateComponent implements OnInit {
  ...
  procedureStepTemplate: ProcedureStepTemplateModel = new ProcedureStepTemplateModel();
  ...
  setProcedureTemplateForEditOrCreate() {
    this.route.queryParams.subscribe(params => {
      this.procedureStepTemplate.id = params['id'] == null ? 0 : Number(params['id']);
  ...
  onSubmit(isUpdate: boolean) {
    ...
    createRequest.referenceProcedures = this.procedureStepTemplate.referenceProcedures;
    ...
    if (isUpdate) {
      ...
      this.procedureStepTemplateService.procedureStepTemplatePatch(env.apiVersion, updateRequest).subscribe(responseHandler((response) => {
        this.router.navigate(['app/procedures/proceduretemplates']);
      }));
    } else {
      this.procedureStepTemplateService.procedureStepTemplatePost(env.apiVersion, createRequest).subscribe(responseHandler((response) => {
        this.router.navigate(['app/procedures/proceduretemplates']);
      }));
    }
  }
