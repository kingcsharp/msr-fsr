...
import {
  ...
  DocumentService,
  RoleRequest,
  CreateProcedureStepRequest,
  Role,
  FileModel,
  ICreateProcedureStepRequest,
  IUpdateProcedureStepRequest,
...
export class ProcedureEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.procedureTemplateService
      .procedureTemplateGet(null, env.apiVersion)
      .pipe(take(1))
      .subscribe(
        responseHandler((procedureTemplateGetResponse) => {
          this.availableProcedureStepTemplates =
            procedureTemplateGetResponse.object.map((s) => ({
              label: s.title,
              value: s.id,
            }));
          ...
          this.roleService
            ...
            .pipe(take(1))
            .subscribe(
              responseHandler((roleResponse) => {
                this.availableRoles = roleResponse.object.sort((a, b) =>
                  a.name > b.name ? 1 : -1
                );
                ...
                this.procedureTypeService
                  ...
                  .pipe(take(1))
                  .subscribe((procedureTypeGetResponse) => {
                    this.availableProcedureTypes =
                      procedureTypeGetResponse.object.map((s) => {
                        if (s.name === "Conformance Action (NCR)") {
                          this.ncrProcedureType = s;
                        }
                        return { label: s.name, value: s.id };
                      });

                    this.globals.showLoader(true);
                    this.procedureStepTypeService
                      .procedureStepType(null, env.apiVersion)
                      .pipe(take(1))
                      .subscribe(
                        responseHandler((procedureStepTypeResponse) => {
                          this.procedureStepTypeOptions =
                            procedureStepTypeResponse.object.map((s) => ({
                              label: s.name,
                              value: s.id,
                            }));
                            ...
                            .subscribe(
                              responseHandler((documentServiceResponse) => {
                                this.documentsAvailable =
                                  documentServiceResponse.object.map((s) => ({
                                    label: s.name,
                                    value: s.id,
                                  }));
                                this.getProcedure();
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      if (this.procedure.id !== 0) {
        ...
        this.procedureService
          ...
          .pipe(take(1))
          .subscribe(
            responseHandler((procedrueGetResponse) => {
  ...
  getProcedureSteps() {
    ...
    this.procedureService
      ...
      .pipe(take(1))
      .subscribe(
        responseHandler((getGetResponse) => {
          this.lastSavedProcedureStepOrder = getGetResponse.object?.map(
            (s) => s.id
          );
          getGetResponse.object.forEach((procedureStep) => {
            ...
            procedureStep.referenceDocumentIds?.forEach(
              (referenceDocumentId) => {
                let document = this.documentsAvailable.find(
                  (s) => s.value === referenceDocumentId
                );
            ...
            if (procedureStep.printOrder !== 1) {
              procedureStep.predecessorStepId = getGetResponse.object.find(
                (s) => s.printOrder === procedureStep.printOrder - 1
              )?.id;
              procedureStep.predecessorStepName = getGetResponse.object.find(
                (s) => s.printOrder === procedureStep.printOrder - 1
              )?.title;
            } else {
            ...
            procedureStep.roles?.forEach((role) => {
              procedureStep.selectedRoles.push(
                this.availableRoles.find((s) => s.id === role.id)
              );
            });
          });

          this.procedureSteps = getGetResponse.object.sort((a, b) =>
          ...
          this.procedureSteps.forEach((procedureStep) => {
            procedureStep.selectedProcedureStepTypeId =
              this.procedureStepTypeOptions.find(
                (s) => s.value === procedureStep.procedureStepTypeId
              )?.value;
  ...
  updateProcedureStep(procedureStep: any) {
    let updateProcedureStepRequest = new UpdateProcedureStepRequest({
      ...
      roles: procedureStep.selectedRoles.map(
        (s) => new RoleRequest({ id: s.id })
      ),
      ...
      referenceDocumentIds: procedureStep.referenceDocuments.map(
        (s) => s.value
      ),
    ...
    this.globals.showLoader(true);
    this.procedureService
      .stepPatch(this.procedure.id, env.apiVersion, updateProcedureStepRequest)
      .pipe(take(1))
      .subscribe(
        responseHandler((response) => {
          let procedureStepsToUpdate = this.lastSavedProcedureStepOrder.filter(
            (s) => s !== procedureStep.id
          );

          let updateAffectedProcedureStepRequests =
            new Array<UpdateProcedureStepRequest>();
          procedureStepsToUpdate.forEach((procedureStepId) => {
            let originalStepIndex = this.lastSavedProcedureStepOrder.findIndex(
              (s) => s === procedureStepId
            );
            let currentStepIndex = this.procedureSteps.findIndex(
              (s) => s.id === procedureStepId
            );

            if (originalStepIndex !== currentStepIndex) {
              this.globals.showLoader(true);
              let procedureStepToUpdate = this.procedureSteps.find(
                (s) => s.id === procedureStepId
              );
              let updateAffectedProcedureStepRequest =
                new UpdateProcedureStepRequest();
              updateAffectedProcedureStepRequest.procedureStepId =
                procedureStepToUpdate.id;
              updateAffectedProcedureStepRequest.equipmentTime =
                procedureStepToUpdate.equipmentTime;
              updateAffectedProcedureStepRequest.laborTime =
                procedureStepToUpdate.laborTime;
              updateAffectedProcedureStepRequest.predecessorStepId =
                procedureStepToUpdate.predecessorStepId;
              updateAffectedProcedureStepRequest.printOrder =
                procedureStepToUpdate.printOrder;
              updateAffectedProcedureStepRequest.procedureId =
                procedureStepToUpdate.procedureId;
              updateAffectedProcedureStepRequest.referenceFiles =
                procedureStepToUpdate.referenceFiles;
              updateAffectedProcedureStepRequest.replacementCost =
                procedureStepToUpdate.replacementCost;
              updateAffectedProcedureStepRequest.roles =
                procedureStepToUpdate.selectedRoles.map(
                  (s) => new RoleRequest({ id: s.id })
                );
              updateAffectedProcedureStepRequest.stepText =
                procedureStepToUpdate.stepText;
              updateAffectedProcedureStepRequest.title =
                procedureStepToUpdate.title;
              updateAffectedProcedureStepRequest.usefulLife =
                procedureStepToUpdate.usefulLife;
              updateAffectedProcedureStepRequest.utilization =
                procedureStepToUpdate.utilization;
              updateAffectedProcedureStepRequest.procedureStepTypeId =
                procedureStepToUpdate.selectedProcedureStepTypeId;
              updateAffectedProcedureStepRequest.printOrder =
                this.procedureSteps.findIndex(
                  (s) => s.id === procedureStepToUpdate.id
                ) + 1;
              updateAffectedProcedureStepRequest.referenceDocumentIds =
                procedureStepToUpdate.referenceDocuments.map((s) => s.value);
              updateAffectedProcedureStepRequests.push(
                updateAffectedProcedureStepRequest
              );
            }

            updateAffectedProcedureStepRequests.forEach(
              (updateAffectedProcedureStepRequestNow) => {
                this.procedureService
                  .stepPatch(
                    this.procedure.id,
                    env.apiVersion,
            ...
            this.lastSavedProcedureStepOrder = this.procedureSteps.map(
              (s) => s.id
            );
          });
        })
  ...
  updateProcedure(procedure: Procedure) {
    procedure.tagType =
      procedure.procedureTypeId === this.ncrProcedureType?.id
        ? procedure.tagType
        : undefined;
    ...
    this.globals.showLoader(true);
    this.procedureService
      .procedurePatch(env.apiVersion, updateProcedureRequest)
      .pipe(take(1))
      .subscribe(() => {});
  }
  ...
  addProcedureStep() {
    if (
      ...
      this.selectedProcedureStepTemplate === null
    ) {
      let createProcedureStepRequest = new CreateProcedureStepRequest({
        ...
        laborTime: 0,
        predecessorStepId:
          this.procedureSteps[this.procedureSteps.length - 1]?.id,
        printOrder:
          this.procedureSteps.length === 0
            ? 1
            : this.procedureSteps[this.procedureSteps.length - 1]?.printOrder +
              1,
      ...
      this.globals.showLoader(true);
      this.procedureTemplateService
        .procedureTemplateGet(
          this.selectedProcedureStepTemplate,
          env.apiVersion
        )
        .pipe(take(1))
        .subscribe(
          responseHandler((response) => {
            let procedureStepTemplateToAdd = <ProcedureStepTemplateModel>(
              response.object[0]
            );
            ...
            let createProcedureStepRequest = new CreateProcedureStepRequest({
              ...
              durationType: null,
              equipmentTime:
                procedureStepTemplateToAdd.equipmentTime === null
                  ? 0
                  : procedureStepTemplateToAdd.equipmentTime,
              laborTime:
                procedureStepTemplateToAdd.laborTime === null
                  ? 0
                  : procedureStepTemplateToAdd.laborTime,
              predecessorStepId:
                this.procedureSteps[this.procedureSteps.length - 1]?.id,
              printOrder:
                this.procedureSteps.length === 0
                  ? 1
                  : this.procedureSteps[this.procedureSteps.length - 1]
                      ?.printOrder + 1,
              procedureId: this.procedure.id,
              replacementCost: procedureStepTemplateToAdd.replacementCost,
              roles: procedureStepTemplateToAdd.roles.map(
                (s) =>
                  new RoleRequest({
                    id: s,
                  } as IRoleRequest)
              ...
              referenceFileIds: procedureStepTemplateToAdd.referenceFiles.map(
                (s) => s.fileId
              ),
              referenceDocumentIds: new Array<number>(),
            } as ICreateProcedureStepRequest);

            this.addProcedureStepToProcedure(
              createProcedureStepRequest,
              procedureStepTemplateToAdd.referenceFiles
            );
          })
        );
    }
  ...
  addProcedureStepToProcedure(
    createProcedureStepRequest: CreateProcedureStepRequest,
    fileModels: Array<FileModel>
  ) {
    this.globals.showLoader(true);
    this.procedureService
      .stepPost(this.procedure.id, env.apiVersion, createProcedureStepRequest)
      .pipe(take(1))
      .subscribe(
        responseHandler((stepPostResponse) => {
          ...
          procedureStepToAdd.durationType =
            stepPostResponse.object.durationType;
          procedureStepToAdd.equipmentTime =
            stepPostResponse.object.equipmentTime;
          ...
          procedureStepToAdd.referenceFiles =
            stepPostResponse.object.referenceFiles;
          procedureStepToAdd.replacementCost =
            stepPostResponse.object.replacementCost;
          ...
          procedureStepToAdd.roles?.forEach((role) => {
            procedureStepToAdd.selectedRoles.push(
              this.availableRoles.find((s) => s.id === role.id)
            );
          });
          ...
          procedureStepToAdd.predecessorStepName =
            this.procedureSteps[this.procedureSteps.length - 1]?.title;
          procedureStepToAdd.selectedProcedureStepTypeId =
            this.procedureStepTypeOptions.find((s) => s.value === 1)?.value;
          procedureStepToAdd.referenceFiles =
            fileModels === undefined ? new Array<FileModel>() : fileModels;
