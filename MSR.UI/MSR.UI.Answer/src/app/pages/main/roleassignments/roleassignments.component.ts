...
import {
  ...
  Role,
  Permission,
  CreateMenuRoleMapRequest,
  UpdateMenuRoleMapRequest,
  ICreateMenuRoleMapRequest,
  IUpdateMenuRoleMapRequest,
...
export class RoleassignmentsComponent implements OnInit {
  ...
  pendingPermissions: UpdatePermissionsEventModel[] =
    new Array<UpdatePermissionsEventModel>();
  ...
  getMenuItems() {
    ...
    this.menuService.menu(env.apiVersion).subscribe(
  ...
  adaptMenuItemsAndRolesToMenuModels(
    ...
    menuItems.map((menuItem) => {
      ...
      roles.map((role) => {
        ...
        roleModel.value =
          menuItem.roles.find((s) => s.id === role.id) != null ? true : false;

        if (roleModel.value) {
          let permissions;
          if (
            menuItem.roles.find((s) => s.id === role.id).permissions != null
          ) {
            permissions = menuItem.roles.find(
              (s) => s.id === role.id
            ).permissions;
          } else {
          ...
          let inheritedPermissions = menuItem.roles.find(
            (s) => s.id === role.id
          ).inheritedPermissions;

          let canActivatePermissionModel = new PermissionModel();
          canActivatePermissionModel.inheritedPermission =
            inheritedPermissions.canActivate;
          canActivatePermissionModel.value =
            permissions.canActivate || inheritedPermissions.canActivate
              ? true
              : false;
          ...
          canApprovePermissionModel.inheritedPermission =
            inheritedPermissions.canApprove;
          canApprovePermissionModel.value =
            permissions.canApprove || inheritedPermissions.canApprove
              ? true
              : false;
          ...
          canCreatePermissionModel.inheritedPermission =
            inheritedPermissions.canCreate;
          canCreatePermissionModel.value =
            permissions.canCreate || inheritedPermissions.canCreate
              ? true
              : false;
          ...
          canDeletePermissionModel.inheritedPermission =
            inheritedPermissions.canDelete;
          canDeletePermissionModel.value =
            permissions.canDelete || inheritedPermissions.canDelete
              ? true
              : false;
          ...
          canEditPermissionModel.inheritedPermission =
            inheritedPermissions.canEdit;
          canEditPermissionModel.value =
            permissions.canEdit || inheritedPermissions.canEdit ? true : false;
          ...
          canReadPermissionModel.inheritedPermission =
            inheritedPermissions.canRead;
          canReadPermissionModel.value =
            permissions.canRead || inheritedPermissions.canRead ? true : false;
        ...
        roleModel.menuItemsChildrenHaveAccessTo =
          this.getMenuItemsChildRolesHaveAccessTo(roleModel.childRoles);

        if (
          roleModel.menuItemsChildrenHaveAccessTo.find(
            (s) => s.id === menuModel.id
          ) !== undefined
        ) {
  ...
  getMenuItemsChildRolesHaveAccessTo(childRoles: Array<Role>) {
    return childRoles
      ...
      .filter(
        (distinctMenuItem, index, menuItemArray) =>
          menuItemArray.findIndex((s) => s.id === distinctMenuItem.id) === index
      );
  }

  addChildRolesToRoleModel(
    roleModel: RoleModel,
    roles: Array<Role>
  ): Array<Role> {
  ...
  roleChanged(menuModule: MenuModel, roleModule: RoleModel) {
    ...
    const pendingRoleChangeIndex = this.pendingPermissions.findIndex(
      (s) =>
        s.menuModule.id === menuModule.id &&
    ...
    if (pendingRoleChangeIndex === -1) {
      ...
      if (roleModule.value) {
        let originalPermissions = this.originalMenuModules
          .find((s) => s.id === menuModule.id)
          .roles.find((m) => m.id === roleModule.id).permissions;
        let menuModulePermissions = this.menuModules
          .find((s) => s.id === menuModule.id)
          .roles.find((m) => m.id === roleModule.id).permissions;
        menuModulePermissions.map((menuModelPermission) => {
          menuModelPermission.value = originalPermissions.find(
            (s) => s.name === menuModelPermission.name
          ).value;
        });
      }
    }
  ...
  updateInheritedRoleAccess(menuModule: MenuModel, roleModule: RoleModel) {
    let rolesWithRoleModuleAsChild = menuModule.roles.filter(
      (s) =>
        s.childRoles.length > 0 &&
        s.childRoles.find((m) => m.id === roleModule.id)
    );

    rolesWithRoleModuleAsChild.map((roleWithRoleModuleAsChild) => {
      roleWithRoleModuleAsChild.childRoleHasAccessToMenuModule =
        this.doesChildRoleHaveAccessToMenuItem(
          roleWithRoleModuleAsChild,
          menuModule
        );
    });
  }

  doesChildRoleHaveAccessToMenuItem(
    roleWithRoleModuleAsChild: RoleModel,
    menuModule: MenuModel
  ) {
    let childRoleHasAccessToMenuItem = false;

    roleWithRoleModuleAsChild.childRoles.map((childRole) => {
      childRoleHasAccessToMenuItem = menuModule.roles.find(
        (s) => s.id === childRole.id
      ).value;
    });
  ...
  permissionChanged(
    ...
    let pendingRoleChangeIndex = this.pendingPermissions.findIndex(
      (s) =>
        s.menuModule.id === menuModule.id &&
        s.roleModule.id === roleModule.id &&
        s.permissionModule === null
    );
    let pendingPermissionChangeIndex = this.pendingPermissions.findIndex(
      (s) =>
        s.menuModule.id === menuModule.id &&
        s.roleModule.id === roleModule.id &&
        s.permissionModule !== null &&
  ...
  generateUniqueUpdateRoleModelRequests(
    permissionChanges
  ): Array<UpdateMenuRoleMapRequest> {
    let uniquePermissionChanges = new Array<UpdateMenuRoleMapRequest>();

    permissionChanges.map((permissionChange) => {
      let uniquePermissionChangeIndex = uniquePermissionChanges.findIndex(
        (s) =>
          s.menuId === permissionChange.menuModule.id &&
          s.roleId === permissionChange.roleModule.id
      );
      ...
      if (uniquePermissionChangeIndex === -1) {
        let updateMenuRoleMapRequest = new UpdateMenuRoleMapRequest({
          ...
          canActivate: permissionChange.roleModule.permissions.find(
            (s) => s.name === "Activate"
          ).value,
          canApprove: permissionChange.roleModule.permissions.find(
            (s) => s.name === "Approve"
          ).value,
          canCreate: permissionChange.roleModule.permissions.find(
            (s) => s.name === "Create"
          ).value,
          canDelete: permissionChange.roleModule.permissions.find(
            (s) => s.name === "Delete"
          ).value,
          canEdit: permissionChange.roleModule.permissions.find(
            (s) => s.name === "Edit"
          ).value,
          canRead: true,
  ...
  savePendingChanges() {
    let roleChanges = this.pendingPermissions.filter(
      (s) => s.permissionModule === null
    );
    let permissionChanges = this.pendingPermissions.filter(
      (s) => s.permissionModule !== null
    );

    let uniqueUpdateRoleModelRequests =
      this.generateUniqueUpdateRoleModelRequests(permissionChanges);
    ...
    if (roleChanges.length === 0) {
      ...
      roleChanges.map((roleChange, index) => {
        if (roleChange.event === "add") {
          ...
          this.menuService
            .rolePost(env.apiVersion, createMenuRoleMapRequest)
            .subscribe(
              responseHandler((response) => {
          ...
          this.menuService
            .roleDelete(
              roleChange.menuModule.id,
              roleChange.roleModule.id,
              env.apiVersion
            )
            .subscribe(
              responseHandler((response) => {
  ...
  updatePermissions(
    uniqueUpdateRoleModelRequests: Array<UpdateMenuRoleMapRequest>
  ) {
    uniqueUpdateRoleModelRequests.map((uniquePermissionChange) => {
      this.menuService
        .rolePatch(env.apiVersion, uniquePermissionChange)
