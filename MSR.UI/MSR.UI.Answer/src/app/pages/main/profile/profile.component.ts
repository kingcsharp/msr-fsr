...
export class ProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.userPwObj = new ResetMyPasswordRequest({
      newPassword: "",
      oldPassword: "",
    });
    ...
    this.timezoneService
      ...
      .subscribe(
        responseHandler((response) => {
          ...
          this.allTimezones = this.allTimezones.sort((a, b) =>
  ...
  submitPwChange() {
    ...
    if (jQuery(jQuery(".parsleyjs")[1]).parsley().isValid()) {
      this.accountService
        .resetmypassword(env.apiVersion, this.userPwObj)
        .pipe(take(1))
        .subscribe(
          responseHandler((response) => {
            this.userPwObj = new ResetMyPasswordRequest({
  ...
  submitDetailsChange() {
    ...
    updateUserReq.customerId =
      updateUserReq.customerId === 0 ? null : updateUserReq.customerId;
    ...
    this.globals.showLoader(true);
    this.userService
      .userPatch(env.apiVersion, updateUserReq)
      .pipe(take(1))
      .subscribe(
        responseHandler((response) => {
