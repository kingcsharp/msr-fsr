...
export class CustomersComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.gridSettings = [
      ...
      new ColumnsSaved({
        id: "primaryContactUser.fullName",
        label: "Primary Contact",
        visible: true,
      }),
      new ColumnsSaved({
        id: "secondaryContactUser.fullName",
  ...
  hasPrivilege(privName) {
    return this.globals.hasPrivilege(
      EnumMenuItem.CustomersDepartments,
      privName
    );
  }
  ...
  getCustomers(event: LazyLoadEvent) {
    ...
    setTimeout(() => {
      callFunctionWithFilters(
        ...
        this.customerService.customerGet,
        event,
        ...
        .subscribe(
          responseHandler((response) => {
            ...
            this.statusOptions = this.data
              .filter(
                (thing, i, arr) =>
                  arr.findIndex((t) => t.status === thing.status) === i
              )
  ...
  deleteCustomer() {
    ...
    this.globals.showLoader(true);
    this.customerService
      .customerDelete(this.customerToDelete.id, env.apiVersion)
      .subscribe(
        responseHandler((response) => {
          this.data.length = 0;
          // TODO FIX THIS SHIT
  ...
  changeCustomerStatus(customer: CustomerModel) {
    ...
    updateCustomerRequest.primaryContactUserId =
      customer.primaryContactUser?.id;
    updateCustomerRequest.secondaryContactUserId =
      customer.secondaryContactUser?.id;
