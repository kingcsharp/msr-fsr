...
export class WipdetailsComponent implements OnInit {
  ...
  completeRemainingSteps() {
    ...
    if (indexOfCurrentWorkOrderInProgress < this.workOrderModel.workOrderTasks.length) {
      this.completThisStep(indexOfCurrentWorkOrderInProgress);
    }
  }

  completThisStep(index) {
    ...
    this.workOrderTaskService.workOrderTaskPatch(env.apiVersion, updateWorkOrderTaskRequest).pipe(take(1)).subscribe(responseHandler(() => {
      ...
      if (index < this.workOrderModel.workOrderTasks.length - 1) {
        this.completThisStep(index + 1);
      }
    }));
  }
  ...
  cancelRemainingSteps() {
    ...
    if (indexOfCurrentWorkOrderInProgress < this.workOrderModel.workOrderTasks.length) {
      this.cancelThisStep(indexOfCurrentWorkOrderInProgress);
    }
  }

  cancelThisStep(index) {
    let workOrderTaskToClose = this.workOrderModel.workOrderTasks[index];
