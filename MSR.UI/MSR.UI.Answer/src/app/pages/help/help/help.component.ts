...
export class HelpComponent implements OnInit {
      ...
      callFunctionWithFilters(
        ...
        .subscribe(
          responseHandler((response) => {
            ...
            let distinctRolesFromReturnedResults = response.object
              .map((s) => s.roles)
              .flat()
              .map((role) => ({ label: role.name, value: role.name }))
              .filter(
                (value, index, self) =>
                  self.findIndex((role) => role.label === value.label) === index
              );
            this.allRoles = this.allRoles.concat(
              distinctRolesFromReturnedResults
            );
          })
        );
    }, 10);
  ...
  deleteHelpPage() {
    ...
    this.globals.showLoader(true);
    this.helpService
      .helpDelete(this.helpPageToDelete.id, env.apiVersion)
      .subscribe(
        responseHandler((response) => {
