...
export function removeDotAndCamelCaseFromObj(elem: any) {
  ...
  Object.keys(elem).forEach((key) => {
    ...
    const keysCapitalized = keysplitted
      .map((x: string, index: number) =>
        index < 1 ? lowerCaseFirstLetter(x) : capitalizeFirstLetter(x)
      )
      .join("");
...
export function callFunctionWithFilters(
  service,
  func,
  event: LazyLoadEvent,
  globalDic,
        ...
        if (typeof filterObj.value[0] === "number") {
          ...
          if (typeof filterObj.value[0] === "object") {
            if (filterObj.value[0] instanceof Date) {
              const parentRoles = filterObj.value
                .filter((s) => s !== null)
                .map((m) => new Date(m));
            ...
            if (
              filterObj.value instanceof Array &&
              moment(filterObj.value[0]).isValid()
            ) {
              argsToCallFn.push(
                filterObj.value
                  .filter((s) => s !== null)
...
export function callFunctionWithFiltersViews(
  service,
  func,
  extraParams: any,
  ...
  filterEvObj.sort.push(
    new Sort({
      ...
      field: capitalizeFirstLetter(
        removeDotAndCamelCaseFromStr(
          event.sortField ? event.sortField : columnsSaved[0].id
        )
      ),
    })
  );
  ...
  Object.keys(uiFilters).forEach((filter: any) => {
    ...
    filterEvObj.filters.push(
      new Filter({
        field: filter,
        value: getValueByType(
          filterObj.value,
          columnsSaved.find((x) => x.id === filter)
        ),
...
export function getOperatorByColumn(
  filtername: any,
  columnsSaved: ColumnsSaved[]
) {
  /*possible operators":
