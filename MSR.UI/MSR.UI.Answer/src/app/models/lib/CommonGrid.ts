...
export class CommonGrid {
  ...
  addView(view: ViewSaved) {
    if (
      this.views.findIndex(
        (x) => x.viewName === view.viewName && x.gridId === view.gridId
      ) !== -1
    ) {
      this.toastr.error(
        `Sorry a view with the name ${view.viewName} alread exists.`
      );
      return;
    }
  ...
  setAsDefault(view: ViewSaved, showSuccess: boolean = true) {
    ...
    if (showSuccess) {
      this.toastr.success(
        `View ${view.viewName} status changed to ${
          view.isDefault ? "Default" : "Not Default"
        }.`
      );
    }
  }
  ...
  updateView(templateView: ViewSaved, currentView: ViewSaved) {
    let view: ViewSaved = this.views.find(
      (x) =>
        x.gridId === templateView.gridId && x.viewName === templateView.viewName
    );
    ...
    localStorage.setItem("viewsSaved", JSON.stringify(this.views));
    this.toastr.success(
      `View ${view.viewName} has been updated to current view.`
    );
  }
  ...
  getDefaultView(gridId: string, gridVersion: string): ViewSaved {
    ...
    if (viewIndex === -1) {
      ...
      const views = this.views.filter(
        (x) => x.gridId === gridId && x.version !== gridVersion
      );
      ...
      this.toastr.warning(
        `Current grid was updated and old views will need to be re-created. Sorry for any inconvenience`
      );
      return null;
    } else {
