...
import {
  ...
  ICreateEquipmentMaintenanceRequest,
...
export class EmPmButtonWrapperComponent implements OnInit {
    ...
    this.locationService
      ...
      .subscribe(
        responseHandler((locationGetResponse) => {
          this.locationOptions = locationGetResponse.object.map((s) => ({
            label: s.name,
            value: s.id,
          }));
          ...
          this.userService
            ...
            .subscribe(
              responseHandler((response) => {
                this.userOptions = response.object.map((s) => ({
                  label: s.fullName,
                  value: s.id,
                }));
              })
  ...
  submitEmPm() {
    ...
    let createEqupmentMaintainaceRequest =
      new CreateEquipmentMaintenanceRequest({
        locationId: this.equipmentMaintenanceModel.locationId,
        statusId:
          this.equipmentMaintenanceModel.statusId === undefined
            ? 9
            : this.equipmentMaintenanceModel.statusId,
        ...
        pemLastCompletedDate:
          this.equipmentMaintenanceModel.pemLastCompletedDate,
    ...
    this.equipmentMaintenanceService
      .equipmentMaintenancePost(
        env.apiVersion,
        createEqupmentMaintainaceRequest
      )
      .subscribe(() => {});
  }
  ...
  lookUp() {
    ...
    this.locationService
      ...
      .subscribe(
        responseHandler((response) => {
          this.equipmentMaintenanceModel.locationId = response.object.find(
            (s) => s.internalAddress === this.internalAddress
          ).id;
        })
      );
  }
