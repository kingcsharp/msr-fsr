...
export class WipstatusWrapperComponent implements OnInit {
    ...
    this.workOrderService
      ...
      .subscribe(
        responseHandler((response) => {
          ...
          this.locationOptions = this.workOrderStatuses
          ...
          if (this.isDisplayedInWipList) {
            ...
            if (
              localStorage.getItem("wipstatus") === undefined ||
              localStorage.getItem("wipstatus") === null
            ) {
              this.selectedLocations = this.locationOptions.map((s) => s.value);
              localStorage.setItem(
                "wipstatus",
                this.selectedLocations.toString()
              );
            } else {
              let locationsAlreadySaved = localStorage
                .getItem("wipstatus")
                .split(",");
              ...
              localStorage.setItem(
                "wipstatus",
                newlocationSavedList.toString()
              );
              this.selectedLocations = this.locationOptions
                .map((s) => s.value)
                .filter((m) => newlocationSavedList.includes(m));
  ...
  displayWorkOrderByProduct(
    workOrderStatuses: Array<any>,
    workOrderStatus: any
  ) {
    let workOrderStatusToUse = workOrderStatuses.find(
      (s) =>
    ...
    if (workOrderStatusToUse === undefined) {
      ...
      newWorkOrderStatus.workOrderSummaries.push(
        workOrderStatus.workOrderSummary
      );

      workOrderStatuses.push(newWorkOrderStatus);
    } else {
      workOrderStatusToUse.workOrderSummaries.push(
        workOrderStatus.workOrderSummary
      );
    }
  }
  ...
  workOrderStatusUpdate(data): void {
    ...
    for (
      ...
      for (
        ...
        if (
          product.workOrderSummaries[summaryIndex].workOrderId !==
          data.workOrderId
        ) {
          continue;
        }
    ...
    if (
      ...
      data.workOrderSummary = {
        ...
        workOrderItemNumber:
          data.customerName.toUpperCase() + "-" + data.workOrderId,
  ...
  openTakeOverAsUserConfirmationDialog(
    workOrderId: number,
  ...
  locationsSelectedUpdated() {
    ...
    this.displayWorkOrderStatuses = this.workOrderStatuses.filter((s) =>
      this.selectedLocations.includes(s.locationName)
    );
  }
  ...
  isLate(currDate): boolean {
    ...
    return (
      this.workOrderStatuses.filter(
        (workOrderStatus: any) => thisDate < this.currentDate
      ).length > 0
    );
  }
  isWarning(currDate): boolean {
    ...
    return (
      this.workOrderStatuses.filter(
        (workOrderStatus: any) =>
          thisDate <= this.warningDate && !(thisDate < this.currentDate)
      ).length > 0
    );
  }
}
