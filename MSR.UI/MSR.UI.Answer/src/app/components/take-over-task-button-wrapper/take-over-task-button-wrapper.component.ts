...
import {
  UpdateWorkOrderTaskRequest, WorkOrderService, WorkOrderModel, WorkOrderTaskModel, WorkOrderTaskService, UserService, UserModel, IUpdateWorkOrderTaskRequest, TakeOverWorkOrderRequest, ITakeOverWorkOrderRequest
  ...
  takeOverWorkOrder(){
    ...
    this.workOrderService.takeOver(env.apiVersion,takeOverWorkOrderRequest).pipe(take(1)).subscribe(takeOverWorkOrderResponse => {
      ...
      workOrdersTaksTakenOver.forEach(workOrderTaskTakenOver => {
        ...
        let workOrderTaskToTakeOver = this.workOrderModel.workOrderTasks.find(s => s.id === workOrderTaskTakenOver.id);
        workOrderTaskToTakeOver.assignedTo = workOrderTaskTakenOver.assignedTo;
        workOrderTaskToTakeOver.assignedToUser = workOrderTaskTakenOver.assignedToUser;
