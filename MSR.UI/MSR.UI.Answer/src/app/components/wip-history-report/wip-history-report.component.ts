...
export class WipHistoryReportComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.workOrderTasks.map((workOrderTask) => {
      if (
        workOrderTask.status.name === "Completed" ||
        workOrderTask.status.name === "Cancelled" ||
        workOrderTask.status.name === "Complete"
      ) {
        workOrderTask.completedOn = workOrderTask.lastUpdatedOn;
      }

      let formattedTotalTaskTime = moment
        .utc(Number(workOrderTask.totalTaskTime) * 1000)
        .format("HH:mm:ss");
      workOrderTask.taskTime =
        formattedTotalTaskTime === "00:00:00" ? "" : formattedTotalTaskTime;
    });
  }
}
