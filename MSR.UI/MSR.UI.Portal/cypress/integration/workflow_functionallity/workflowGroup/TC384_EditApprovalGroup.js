describe('Workflow Group Functionality', () => {
    it('TC378_ApprovalWorkflowGrid', () => {
        ...
        cy.get('[data-cy=user-header]').contains('Edit Workflow Group');
        cy.get('[data-cy=name-input]').clear();
        cy.get('[data-cy=name-input]').type('Workflow Test2').should('have.value', 'Workflow Test2');

        cy.multiselectFormClick('form-groupRoles','CEO');
        cy.multiselectFormClick('form-groupUsers','Tasha Butcher');

        cy.get('[data-cy=save-button]').click();
        //end edit
        cy.get(".ui-blockui-document", { timeout: 4000 }).should("not.be.visible");
        cy.get('[data-cy=searchbyname-grid]').clear();
        cy.get('[data-cy=searchbyname-grid]').type('Workflow Test2').should('have.value', 'Workflow Test2');
        cy.get('tbody').find('tr:first-child td').contains('Workflow Test2');
