...
namespace Answer.Web.Controllers
{
    ...
    public class FilesController : BaseController
    {
        ...
        public ActionResult LatestFilesData(JqGridParam param)
        {
            var fileService = new FileService();

            var totalRows = fileService.GetLatestFilesQueryable();

            if (param.where != null && param.where.rules.Any())
            {
                foreach (var rule in param.where.rules)
                {
                    if (rule.field == nameof(LatestFileView.NAME))
                    {
                        totalRows = totalRows.Where(x => x.NAME.ToLower().Contains(rule.data.ToLower()));
                    }
                    else if (rule.field == nameof(LatestFileView.CONTENTTYPE))
                    {
                        totalRows = totalRows.Where(x => x.CONTENTTYPE.ToLower().Contains(rule.data.ToLower()));
                    }
                }
            }
            ...
            var totalPages = (int)Math.Ceiling((float)totalRecords / param.pageSize);
