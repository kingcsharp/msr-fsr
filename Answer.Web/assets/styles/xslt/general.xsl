...
<html>
	...
	<body>
		<xsl:attribute name="onLoad"><xsl:call-template name="setUpHideColumns" />resetSelectClose();resetTextAdder();onLoadFunction();startLogOutTimer();<xsl:if test="/Doc_Webpage/queryString/item[@name='showToolBox']/@value='true' or /Doc_Webpage/whoAmI/record/field[@name='TOOL_BOX']/@value='1'">showRowActionItems();</xsl:if><xsl:if test="/Doc_Webpage/pageName[@val='mainMenu.asp']">showWarnings ();</xsl:if><xsl:if test="/Doc_Webpage/pageName[@val='editProcedure.asp']">initialiseWebWizRTE();</xsl:if><xsl:for-each select="/Doc_Webpage/onLoadDoThis"><xsl:value-of select="."/></xsl:for-each></xsl:attribute>
	...
	<table class="tight" width="100%" style="margin:0">
		<tr>
			...
			<td class="new_body" style="text-align: left;font-size: x-small;">
                ...
                For assistance using this application, call +1 (703) 840-3040 or <a href="mailto:support@cmhworks.com">Email Support</a>
							</xsl:when>
							<xsl:otherwise>
 ...
 <head>
    ...
    <script>
      <xsl:attribute name="src">
        <xsl:value-of select="$path_to_lib"/>
        <xsl:text>asp/common/javascript/deny.js</xsl:text>
      </xsl:attribute>
      //placeholder
    </script>
    <script>
      <xsl:attribute name="src">
        <xsl:value-of select="$path_to_lib"/>
        <xsl:text>asp/common/javascript/jquery.simplemodal.1.4.4.min.js</xsl:text>
      </xsl:attribute>
      //placeholder
    </script>
    <script>
    ...
    <script>
		<xsl:attribute name="src"><xsl:value-of select="$path_to_lib"/>asp/common/javascript/jquery.qrcode.js</xsl:attribute>
		//placeholder
	</script>
	<script>
		<xsl:attribute name="src"><xsl:value-of select="$path_to_lib"/>asp/common/javascript/qrcode.js</xsl:attribute>
		//placeholder
	</script>
	</xsl:if>
...
<!-- Style for layers -->
   ...
   <script>
     $(function() {
     $( document ).tooltip({ show:{effect:"drop",duration: 300}, hide: { effect: "drop", duration: 300 }, tooltipClass: "tooltip" , classes: "tooltip"  });
     });
   </script>
...
<select>
  ...
  <option value="DELIVERY_SCREEN">
    <xsl:call-template name="putText">
      <xsl:with-param name="key">Delivery Screen</xsl:with-param>
    </xsl:call-template>
  </option>
  <option value="NCR_SEARCH">
    <xsl:call-template name="putText">
      <xsl:with-param name="key">NCR Search</xsl:with-param>
    </xsl:call-template>
  </option>
