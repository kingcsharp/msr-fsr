...
<xsl:for-each select="purchaseItems/record">
  ...
  <xsl:call-template name="PRINT_TECHNICAL_SERVICE_REPORT">
...
<xsl:param name="t"/>
  <xsl:call-template name="TSR_HEADER">
	<xsl:with-param name="t" select="$t" />
	<xsl:with-param name="fill" select="$t/fillData/record" />	
</xsl:call-template>
</xsl:template>
...
<table class="tsrHeader">
  ...
  <xsl:if test="not(/Doc_Webpage/queryString/item[@name='TSR_TYPE']/@value = 'WIP_HIST')">
    <xsl:call-template name="printProcedure">
	    <xsl:with-param name="procID"><xsl:value-of select="$fill/PURCHASE_HIST_ID" /></xsl:with-param>
	    <xsl:with-param name="fill" select="$fill" /><xsl:with-param name="t" select="$t" />
      
    </xsl:call-template>
  </xsl:if>
  <xsl:call-template name="printTaskSummary"></xsl:call-template>
</xsl:template>
...
<table>
  ...
  <xsl:template name="printTaskSummary">
    <table style="margin:auto">
    <xsl:for-each select="TASK_DATA/record">
      <tr >
        ...
        <td class="normalTable">
          <xsl:copy-of select="STEP_TEXT_ALL_HTML"/>
          ...
          <xsl:for-each select="../../monitors/record[TASK_ID = $taskID]">
            <div class="monitor" >
              ...
              <xsl:call-template name="putText">
                <xsl:with-param name="key">Monitor:</xsl:with-param>
              </xsl:call-template>
