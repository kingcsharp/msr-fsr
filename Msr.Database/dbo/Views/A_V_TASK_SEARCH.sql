...
SELECT     TASK.DESCRIPTION, TASK.STATUS, TASK.REQUESTOR, TASK.CHILD_ORDER, TASK.CREATED_BY, TASK.CREATE_DATE, dbo.leadingSpaces(TASK.ID, 
                      ...
                      TASK.COLOR_CODE, disc.DISCUSSION_ID, sur.SURVEY_ID, TASK.LAST_REQUEST_DATE, mee.MEETING_ID, dbo.getTaskParentList(TASK.ID) 
                      AS PARENT_LIST, TASK.PARENT_ID, dbo.isParentTask(TASK.ID) AS isParent, TASK.PRIORITY, 
                      dbo.A_TASK_PRIORITY_LEVELS.NAME AS PRIORITY_NAME, dbo.A_PEOPLES_FAVORITES.[GROUP] AS GROUP_ID, 
                      dbo.A_PEOPLES_FAVORITE_GROUPS.FAV_TYPE, dbo.A_PEOPLES_FAVORITE_GROUPS.GROUP_NAME, 
                      dbo.A_PEOPLES_FAVORITE_GROUPS.PERSON, dbo.A_TASK_COMPANIES_TO_VIEW_LINK.CO_ID AS ALLOWED_CO_ID, 
...
FROM         dbo.A_V_PEOPLE_DATA_QUICK ASSIGNEE INNER JOIN
                      ...
                      dbo.A_PEOPLES_FAVORITES INNER JOIN
                      dbo.A_PEOPLES_FAVORITE_GROUPS ON dbo.A_PEOPLES_FAVORITES.[GROUP] = dbo.A_PEOPLES_FAVORITE_GROUPS.ID ON 
                      TASK.ID = dbo.A_PEOPLES_FAVORITES.ITEM LEFT OUTER JOIN
                      dbo.A_TASK_PRIORITY_LEVELS ON TASK.PRIORITY = dbo.A_TASK_PRIORITY_LEVELS.ID LEFT OUTER JOIN
                      dbo.A_TASK_MEETING_LINK mee ON TASK.ID = mee.TASK_ID LEFT OUTER JOIN
                      dbo.A_TASK_DISCUSSION_LINK disc ON TASK.ID = disc.TASK_ID LEFT OUTER JOIN
